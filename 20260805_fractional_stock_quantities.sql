-- TBR System: support fractional stock quantities with three decimal places.
-- Apply after 20260804_atomic_requisition_and_write_hardening.sql.
...
CREATE OR REPLACE FUNCTION create_requisition_atomic(
  p_job_id UUID, p_no TEXT, p_items JSONB, p_note TEXT DEFAULT NULL
) RETURNS requisitions
LANGUAGE plpgsql SECURITY DEFINER SET search_path = public
AS $$
DECLARE
  v_requisition requisitions; v_item JSONB; v_stock stock_items; v_qty NUMERIC;
  v_clean_items JSONB := '[]'::JSONB;
BEGIN
  IF auth.uid() IS NULL OR current_app_role() NOT IN ('admin','supervisor','technician') THEN RAISE EXCEPTION 'Not permitted'; END IF;
  ...
  FOR v_item IN SELECT value FROM jsonb_array_elements(p_items) LOOP
    ...
    IF v_qty<=0 OR scale(v_qty)>3 OR v_qty>999999999.999 THEN RAISE EXCEPTION 'Stock quantity must be positive with at most three decimal places'; END IF;
    v_stock:=NULL;
    IF NULLIF(v_item->>'sid','') IS NOT NULL THEN
      ...
      IF COALESCE(v_stock.quantity,0)<v_qty THEN RAISE EXCEPTION 'Insufficient stock for % (available %, requested %)',v_stock.sku,v_stock.quantity,v_qty; END IF;
...
CREATE OR REPLACE FUNCTION update_requisition_atomic(
  p_requisition_id UUID, p_items JSONB, p_note TEXT DEFAULT NULL
) RETURNS requisitions
LANGUAGE plpgsql SECURITY DEFINER SET search_path = public
AS $$
DECLARE
...
BEGIN
  ...
  IF current_app_role() NOT IN ('admin','supervisor') AND v_requisition.created_by IS DISTINCT FROM auth.uid() THEN RAISE EXCEPTION 'Not permitted'; END IF;
  IF jsonb_array_length(COALESCE(p_items,'[]'::JSONB))=0 THEN RAISE EXCEPTION 'Requisition requires items'; END IF;
...
RETURNS requisitions LANGUAGE plpgsql SECURITY DEFINER SET search_path = public
AS $$
...
REVOKE ALL ON FUNCTION create_requisition_atomic(UUID,TEXT,JSONB,TEXT) FROM PUBLIC;
REVOKE ALL ON FUNCTION update_requisition_atomic(UUID,JSONB,TEXT) FROM PUBLIC;
REVOKE ALL ON FUNCTION delete_requisition_atomic(UUID) FROM PUBLIC;
